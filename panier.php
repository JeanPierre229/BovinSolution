        ...
        <form action="panier.php" method="post" class="formulaire">
            <div class="row my-3 p-2 justify-content-between">
                <div class="col-lg-8 col-md-12 col-12 row-form px-5 py-4">
                    ...
                    <?php while($user_panier = $requete2->fetch()): ?>
                        <div class="col">
                            <img src="images/<?php echo htmlspecialchars($user_panier['image']); ?>" alt="Grazing Services Package" class="img w-100 rounded-5">
                        </div>
                        <div class="col-2 my-3">
                            <h6><strong><?php echo htmlspecialchars($user_panier['nom_produit']); ?></strong></h6>
                            <h6 class="">Beige</h6>
                        </div>
                        ...
                        <div class="col-3 my-5 text-center">
                            <input type="number" min="1" class="px-2 rad col-4 input-form quantite" name="quantite1" id="quantite<?php echo $user_panier['id']; ?>" placeholder="1" required>
                        </div>
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const quantiteInput = document.getElementById('quantite<?php echo $user_panier['id']; ?>');
                                quantiteInput.addEventListener('input', function(event) {
                                    const val = event.target.value;
                                    const prixUnitairePHP = <?php echo $user_panier['prix']; ?>; // Récupérer le prix unitaire PHP
                                    const prixTotal = val * prixUnitairePHP; // Calculer le prix total
                                    document.getElementById("prixTotalPHP<?php echo $user_panier['id']; ?>").textContent = prixTotal; // Afficher le prix total dans la page HTML
                                });
                            });
                        </script>
                        <div class="col my-5">
                            <div class="row">
                                <div class="col-6" id="prixTotalPHP<?php echo $user_panier['id']; ?>"><?php echo $user_panier['prix']; $sous_total += $user_panier['prix']; ?></div>
                                <div class="col-6 text-end">
                                    <a href="delete_product.php?id=<?php echo $user_panier['id']; ?>">
                    ...
                    <div class="col">
                        <div class="row">
                            ...
                            <div class="col-2 text-start">
                                <script>
                                    document.getElementById("prixTotalPHP<?php echo $user_panier['id']; ?>").textContent = prixTotal;
                                </script>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-10 text-end"><strong>Livraison:</strong></div>
                            <div class="col-2 text-start">1000 FCFA</div>
                        </div>
                        <div class="row my-3">
